...
# Base de dados de utilizadores autorizados e respetivas senhas e perfis
# 'admin' -> Acesso total (pode apagar)
# 'normal' -> Pode cadastrar, vender e editar, mas NÃO pode apagar
UTILIZADORES = {
...
# Função de Autenticação com Múltiplos Utilizadores
def verificar_autenticacao():
  ...
  if not st.session_state["autenticado"]:
    ...
    with st.form("form_login"):
      utilizador_digitado = st.text_input("Utilizador:")
      senha_digitada = st.text_input(
          "Palavra-passe (password):", type="password"
      )
      btn_entrar = st.form_submit_button("Entrar")

      if btn_entrar:
        if (
            utilizador_digitado in UTILIZADORES
            and UTILIZADORES[utilizador_digitado]["senha"] == senha_digitada
        ):
          st.session_state["autenticado"] = True
          st.session_state["utilizador_atual"] = UTILIZADORES[
              utilizador_digitado
          ]["nome"]
          st.session_state["perfil_atual"] = UTILIZADORES[utilizador_digitado][
              "perfil"
          ]
          st.rerun()
        else:
...
with aba_stock:
  ...
  # Se for utilizador normal, restringe as opções visíveis (retira 'Eliminar')
  if st.session_state["perfil_atual"] == "admin":
    opcoes_stock = ["Novo", "Nova Entrada", "Editar", "Eliminar"]
  else:
    opcoes_stock = ["Novo", "Nova Entrada", "Editar"]

  acao_produto = st.radio("Operação:", opcoes_stock, horizontal=True)
...
with aba_historico:
  ...
  if df_vendas.empty:
    ...
    # Apenas o admin pode ver a opção de apagar vendas no histórico
    if st.session_state["perfil_atual"] == "admin":
